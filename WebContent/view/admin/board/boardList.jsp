...
<script>
	function goWrite() {
		location.href = '../admin/boardWriteFrmAction.yo';
	}
</script>
</head>
<body>
	...
	<table class="bbs">
		...
		<tr>
			<td>공지</td>
			<td>
				 <a href="../admin/boardDetailAction.yo?num=${notice.bidx}&pageNum=${spage}">${notice.subject}</a>
			</td>
		...
		<tr>
			<td>${board.bidx}</td>
			<td>
				 <a href="../admin/boardDetailAction.yo?num=${board.bidx}&pageNum=${spage}">${board.subject}</a>
			</td>
	...
	<c:if test="${fn:length(requestScope.boardList) > 0}">
		<c:if test="${startPage != 1}">
			<a href='../admin/boardListAction.yo?pageNum=${startPage-1}'>[ 이전 ]</a>
		</c:if>
		...
		<c:forEach var="pageNum" begin="${startPage}" end="${endPage}">
			...
			<c:if test="${pageNum != spage}">
				<a href='../admin/boardListAction.yo?pageNum=${pageNum}'>${pageNum}&nbsp;</a>
			</c:if>
		</c:forEach>
		
		<c:if test="${endPage != maxPage }">
			<a href='../admin/boardListAction.yo?pageNum=${endPage+1 }'>[다음]</a>
		</c:if>
	</c:if>
